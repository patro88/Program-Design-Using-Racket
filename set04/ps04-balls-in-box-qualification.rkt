...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ps04-balls-in-box-qualification) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
(require rackunit)
...
(define world-1 (initial-world 8))
...
(define-test-suite balls-in-box-tests
  ...
  (check-provided world-2)  ;; this already tests world-after-key-event
  (check-provided (world-after-mouse-event world-1 100 100
                    "button-down"))
  (check-provided (world-after-tick world-1))
